...
class SensorData:
    def __init__(self, x, y, z, temp, time, ix):
        self.x = x
        self.y = y
        self.z = z
        self.temp = temp
...
def Update(frame):
    ...
    while ser.in_waiting:
        ...
        if len(raw_data) == 14:
            ...
            glava, counter, timer, x, y, z, temp = struct.unpack('<7h', raw_data)
            print(f"header: {glava} count: {counter}  timer: {timer} koti:({x}, {y}, {z}), temp: {temp}")

            data_array.append(SensorData(x, y, z, temp, current_time, counter))
    ...
    Temp_data = [data.temp for data in data_array]
    ...
    line4.set_data(Counter_data, Temp_data)
    ax1.relim()
    ax1.autoscale_view()
    ax2.relim()
    ax2.autoscale_view()
...
line4, = ax2.plot([], [], label='Temperatura', color='red')
