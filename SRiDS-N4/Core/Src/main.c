...
void initGyro() { // ne pozabit klicat te funkcije
    ...
    spi1_pisiRegister(0x22, 0x08); // Nastavi na CTRL_REG3 -> I2_DRDY
    spi1_pisiRegister(0x23, 0x10); // +-500 DPS
}
/* USER CODE END 0 */
...
int main(void)
{
  ...
  bool toggle = false;
  int16_t paket[7];
  paket[0] = 0xAAAB;// glava za zaznamek zaceteka paketa
  int16_t ix_paket = 0;
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	  ...
	  if (HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_0) == 1) {
		  while (HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_0) == 1); // debounce
		  toggle = !toggle;
	  }
	  ...
	  if(toggle){
		  HAL_Delay(100);
		  paket[1] = ix_paket;
		  paket[2] = (uint16_t)__HAL_TIM_GET_COUNTER(&htim1);
		  spi1_beriRegistre(0x28, (uint8_t*)&paket[3], 6);
		  paket[6] = (uint16_t)(25-spi1_beriRegister(0x26));
		  while(CDC_Transmit_FS((uint8_t*)&paket, 14));
