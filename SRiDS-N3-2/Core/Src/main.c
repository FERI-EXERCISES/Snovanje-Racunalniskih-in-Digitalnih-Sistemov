...
int main(void)
{
  ...
  int arr[] = {0, 2, 4, 8, 16, 32, 64, 128, 256, 512, 1024, 512, 256, 128, 64, 32, 16, 8, 4, 2};
  int counter = 0;
  ...
  while (1)
  {
	...
	if (currentButtonState == GPIO_PIN_RESET && lastButtonState == GPIO_PIN_SET)
	{
		...
		TIM4->CCR1 = arr[counter];
		TIM4->CCR3 = arr[counter];
	...
	if(timer_val < ((arr[counter] * (htim3.Init.Period + 1)) / (1000 - 1)))
